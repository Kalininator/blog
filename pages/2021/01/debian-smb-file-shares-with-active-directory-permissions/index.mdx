...
import { PostMetadata } from '../../../../components/HeadPost.tsx';
...
![smb.conf file](/images/posts/smb-debian/smb_config.png)
...
![/etc/nsswitch.conf](/images/posts/smb-debian/nsswitch.png)

## Joining the domain

At this point, your system should be ready to join your domain. Just make sure you have the DNS configured to use the domain's DNS servers. The next step is to join the domain using `sudo net ads join -U <username>` where the username is for the account that has permission to domain join. In my example, I used my username like so:

![Joining the domain](/images/posts/smb-debian/joining-domain.png)
...
![Creating the folder and setting permissions](/images/posts/smb-debian/creating-folder.png)
...
![Adding the samba config entry](/images/posts/smb-debian/samba-entry.png)
...
![New file share showing in windows](/images/posts/smb-debian/share-in-windows.png)
