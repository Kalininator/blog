...
Overall I really like this new setup. It's a lot simpler and means I can go travelling while still keeping the lab fully functional.
The new colocation box is still very well specced (too much if anything), so there is plenty of room to setup more services.

Hopefully I never have to visit the server in colocation and it just keeps running fine, and its been running seamlessly for a while now so I'm optimistic about it.
For next steps, I should really get round to setting up better monitoring for all this with Grafana/Prometheus, and some alerting.
